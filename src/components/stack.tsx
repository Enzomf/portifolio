...
import { SiTailwindcss, SiMysql, SiMongodb, SiDocker } from 'react-icons/si';
...
export const Stack = ({ stackIcons }: Props) => {
    const scale = [1, 1.1, 1];
    const transform = ["translateY(0px)", "translateY(-4px)", "translateY(0px)"];


    const sequence = stackIcons.map((_, index) => (
        [
            `.circle-${index + 1}`,
            { scale, transform },
            { duration: 1, delay: index * 0 }
        ]
    ));

    sequence.push([
        `.circle-1`, // reanimate first icon for smooth loop
        { scale, transform },
        { duration: 1, delay: stackIcons.length * 0 }
    ]);
...
const Container = ({
    ...
    return (
        <div
            className={cn(
                `h-9 w-9 rounded-full flex items-center justify-center bg-[rgba(248,248,248,0.01)]
